...
void bdProfiles::getPublicInfos(bdServiceServer* server, bdByteBuffer& message)
{
	...
	PMExtProfileData* outData = new PMExtProfileData[npIDs.size()];
	auto async = PMGetExtProfileData(npIDs.size(), "iw5pub", &npIDs[0], outData);
	...
	{
		PMExtProfileData* row = &result->results[i];

		infos[i].npID = row->npID;		
		//infos[i].profileInt = *(int*)(row->buffer);
		infos[i].profileData = std::string(row->buffer, row->bufLength);
...
void bdProfiles::setPublicInfo(bdServiceServer* server, bdByteBuffer& message)
{
	...
	memcpy(buf, info.profileData.c_str(), info.profileData.size());

	PMID npID;
	PM_GetPMID(&npID);

	auto async = PM_WriteUserFile("profileiw5pub", npID, (uint8_t*)buf, info.profileData.size() + 4);
