...
//void WriteServerInformation(DWORD memaddr, DWORD addr, int port, int pswrd, int con_disabled, int pure, int nettype, int clients, int maxclients, int voice, WORD minping, WORD maxping, WORD	ping, int dedicated, char* hostname, char* mapname, char* gametype, int servernum, int unk, int unk2, char*	sv_hostname);
...
struct ProxyResponse : ISteamMatchmakingServerListResponse
{
	...
	{
		Log::Debug("ProxyResponse", "ServerResponded %i", iServer);
	...
	{
		Log::Debug("ProxyResponse", "ServerFailedToRespond %i", iServer);
	...
	{
		Log::Debug("ProxyResponse", "RefreshComplete");
...
{
	Log::Debug("SteamMatchmakingServers", "RequestInternetServerList");
	...
	//DWORD Addr = 0xC86B30;
	//WriteServerInformation(Addr, ConvertIPSTRtoDWORD("192.168.1.102"), 27015, 0, 0, 0, 1, 0, 18, 2, 0, 999, 60, 1, "lelz server", "mp_dome", "war", 1, 1, 304, "lelz server");
...
{
	Log::Debug("SteamMatchmakingServers", "RequestLANServerList");
...
HServerListRequest CSteamMatchmakingServers002::RequestFriendsServerList(AppId_t iApp, MatchMakingKeyValuePair_t **ppchFilters, uint32 nFilters, ISteamMatchmakingServerListResponse *pRequestServersResponse)
{
	...
	isOfficial = true;
	GSClient_QueryMaster();
	Log::Debug("SteamMatchmakingServers", "RequestFriendsServerList");
	return NULL;
}
...
{
	Log::Debug("SteamMatchmakingServers", "RequestFavoritesServerList");
	return NULL;
}
...
{
	Log::Debug("SteamMatchmakingServers", "RequestHistoryServerList");
	return NULL;
}
...
{
	Log::Debug("SteamMatchmakingServers", "RequestSpectatorServerList");
	return NULL;
}
...
{
	Log::Debug("SteamMatchmakingServers", "ReleaseRequest");
}
...
{
	Log::Debug("SteamMatchmakingServers", "GetServerDetails");
...
{
	Log::Debug("SteamMatchmakingServers", "CancelQuery");
}
...
{
	Log::Debug("SteamMatchmakingServers", "RefreshQuery");
}

// Returns true if the list is currently refreshing its server list
bool CSteamMatchmakingServers002::IsRefreshing(HServerListRequest hRequest)
{
	Log::Debug("SteamMatchmakingServers", "IsRefreshing");
	return false;
}
...
{
	Log::Debug("SteamMatchmakingServers", "GetServerCount");
...
{
	Log::Debug("SteamMatchmakingServers", "RefreshServer");
}
...
{
	Log::Debug("SteamMatchmakingServers", "PingServer");
	return NULL;
}
...
{
	Log::Debug("SteamMatchmakingServers", "PlayerDetails");
	return NULL;
}
...
{
	Log::Debug("SteamMatchmakingServers", "ServerRules");
	return NULL;
}
...
{
	Log::Debug("SteamMatchmakingServers", "CancelServerQuery");
}
...
bool GSClient_Init()
{
	...
	{
		Log::Debug("GSClient", "socket() failed - %d", WSAGetLastError());
		return false;
	}
	...
	{
		Log::Debug("GSClient", "bind() failed - %d", WSAGetLastError());
		return false;
	}
...
std::vector <const char*> receivedservers;
...
{
	Log::Debug("GSClient", "received infoResponse");
	DWORD LocalIP = getMyIP();

	DWORD Addr = SVBaseAddress;
	...
	*(BYTE*)SVBaseCount2 = 0;
	...
	for (int i = 0; i < g_cls.numServers; i++)
	{
		...
		if ((server->m_NetAdr.GetIP() == ntohl(g_cls.from.sin_addr.s_addr)) && (server->m_NetAdr.GetQueryPort() == ntohs(g_cls.from.sin_port)))
		{
			...
			//Log::Debug("GSClient", "received *matching* infoResponse - %d %s", i, Info_ValueForKey(buffer, "hostname"));
			...
			//\hc\0\shortversion\1.9\fs_game\modernwarfare3\pure\1\gametype\war\sv_maxclients\-48\sv_privateClients\5\clients\0\mapname\mp_dome\protocol\19816\gamename\PIW5
			...
			strcpy(server->m_szGameDescription, Info_ValueForKey(buffer, "gametype"));
			...
			//Log::Debug("GSClient", "received infoResponse %s ", buffer);
			//Log::Debug("GSClient", "received *matching* infoResponse -%s %d %d", Info_ValueForKey(buffer, "hostname"), server->m_NetAdr.GetQueryPort(), server->m_NetAdr.GetConnectionPort());

			/*Log::Debug("GSClient", (char*)va("addr %d", server->m_NetAdr.GetIP()));
			Log::Debug("GSClient", (char*)va("gameport %d", server->m_NetAdr.GetConnectionPort()));
			Log::Debug("GSClient", (char*)va("Password %d", server->m_bPassword));
			Log::Debug("GSClient", (char*)va("Players %d", server->m_nPlayers));
			Log::Debug("GSClient", (char*)va("MaxPlayers %d", server->m_nMaxPlayers));
			Log::Debug("GSClient", (char*)va("m_nPing %d", server->m_nPing));
			Log::Debug("GSClient", (char*)va("hostname %s", Info_ValueForKey(buffer, "hostname")));
			Log::Debug("GSClient", (char*)va("Map %s", server->m_szMap));
			Log::Debug("GSClient", (char*)va("gametype %s", Info_ValueForKey(buffer, "gametype")));
			Log::Debug("GSClient", (char*)va("hostname %s", Info_ValueForKey(buffer, "hostname")));*/

			char* hostname = (char*)va("%s", Info_ValueForKey(buffer, "hostname"));
			char* mapname = (char*)va("%s", server->m_szMap);
			char* gametype = (char*)va("%s", Info_ValueForKey(buffer, "gametype"));
			int gport = server->m_NetAdr.GetConnectionPort();
			int ping = server->m_nPing;
			int players = server->m_nPlayers;
			int maxplayers = server->m_nMaxPlayers;
			int passwordreq = server->m_bPassword;

			//maxplayers = maxplayers - players;

			DWORD ipAddress = server->m_NetAdr.GetIP();

			ipport newserver;
			newserver.ip = server->m_NetAdr.GetIP();
			newserver.port = gport;

			receivedservers.push_back(va("%d.%d.%d.%d:%d", ((ipAddress >> 24) & 0xFF), ((ipAddress >> 16) & 0xFF), ((ipAddress >> 8) & 0xFF), (ipAddress & 0xFF), gport));

			//WriteServerInformation(Addr, ConvertIPSTRtoDWORD("192.168.1.103"), 27015, 0, 0, 0, 1, 0, 18, 2, 0, 999, 60, 1, "lelz server", "mp_dome", "war", i + 1, 1, 304, "lelz server");
			
			Log::Error("", "Added %d", g_cls.numServers);
			WriteServerInformation(Addr, ReverseIPAddress(ipAddress), gport, passwordreq, 0, 0, 1, players, maxplayers, 2, 0, 999, ping, 1, hostname, mapname, gametype, i + 1, 1, 304, hostname);
			//Addr = Addr + 208;
	...
	remove_duplicate(receivedservers);

	*(BYTE*)SVBaseCount = receivedservers.size();
	*(BYTE*)SVBaseCount2 = receivedservers.size();
...
{
	Log::Debug("GSClient_HandleOOB", "%s", buffer);
...
void GSClient_PollSocket()
{
	...
	while (true)
	{
		...
		if (len == SOCKET_ERROR)
		{
			...
			{
				Log::Debug("GSClient", "recv() failed - %d", error);
			}
...
void GSClient_QueryMaster()
{
	...
	if (!lookedUp)
	{
		...
		//host = gethostbyname("178.22.121.75");
		host = gethostbyname(MASTER_SERVER);

		if (!host)
		{
			Log::Debug("GSClient", "gethostbyname() failed - %d", WSAGetLastError());
			return;
		}
	...
	char message[128];
	//_snprintf(message, sizeof(message), "\xFF\xFF\xFF\xFFgetservers PIW5 19816 full empty");
	_snprintf(message, sizeof(message), "\xFF\xFF\xFF\xFFgetservers PIW5 20601 full empty");
