...
bool SteamProxy_InitInternal()
{
	...
	PM_SetFriendSteamGUID(steamID.ConvertToUint64());
...
void SteamProxy_DoThatTwinklyStuff()
{
	if (Global::Game::Type != GameType::GAME_TYPE_DEDI)
	{
		...
		{
			Log::Debug("SteamProxy", "Skipping Steam authentication.");
			return;
		}

		Com_Printf(0, "Performing Steam authentication...\n");
		Log::Debug("SteamProxy", "Performing Steam authentication...");
		...
		{
			Com_Error(1, "An error occurred during Steam verification. Please try again later. If this issue persists, ask on http://plusmaster.pro/.");
		}
		...
		Log::Debug("SteamProxy", "Steam authentication passed.");
	}	
}