...
static void SteamGS_OnValidateTicket(PMAsync<PMValidateTicketResult>* async)
{
	PMValidateTicketResult* result = async->GetResult();

	Log::Debug("SteamGS", "validateTicketResult %i %llx", result->result, result->id);

	if (result->result == ValidateTicketResultOK)
	{
...
extern CSteamID GetPlayerSteamID();
...
bool CSteamGameServer010::SendUserConnectAndAuthenticate( uint32 unIPClient, const void *pvAuthBlob, uint32 cubAuthBlobSize, CSteamID *pSteamIDUser ) {
	Log::Debug("SteamGS", "sendUserConnectAndAuthenticate 0x%x", unIPClient);

	//PMAsync<PMValidateTicketResult>* async = PM_ValidateTicket(pvAuthBlob, cubAuthBlobSize, unIPClient, pSteamIDUser->ConvertToUint64());
...
{
	//Log::Debug("SteamGameServer", "UpdateServerStatus %i %i %i %s %s %s", cPlayers, cPlayersMax, cBotPlayers, pchServerName, pSpectatorServerName, pchMapName);
}
