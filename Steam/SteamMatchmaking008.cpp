...
const char *CSteamMatchmaking008::GetLobbyData( CSteamID steamIDLobby, const char *pchKey ) {
	//PMID friendID = steamIDLobby.ConvertToUint64();
	PMID friendID = joinedLobbyID.ConvertToUint64();
...
//	PMLobbyData* data;
	...
	/*PMAsync<PMLobbyData>* async = PMGetLobbyData(friendID);
	async->Wait();
	...
	if(pchKey == "lobbyid")
	{
		return async->GetResult()->lobbyid;
	}
	else if(pchKey == "dw_sec_kid")
	{
		MessageBox(0,async->GetResult()->dw_sec_kid,"info",MB_ICONINFORMATION);
		return async->GetResult()->dw_sec_kid;
	}
	else if(pchKey == "dw_sec_key")
	{
	MessageBox(0,async->GetResult()->dw_sec_key,"info",MB_ICONINFORMATION);
		return async->GetResult()->dw_sec_key;
	}
	else if(pchKey == "dw_serialized_addr")
	{
	MessageBox(0,async->GetResult()->dw_serialized_addr,"info",MB_ICONINFORMATION);
		return async->GetResult()->dw_serialized_addr;
	}

	presence = PM_GetLobbyDataValue((char*)pchKey);*/

	presence = PM_GetFriendRichPresence(friendID, pchKey);
