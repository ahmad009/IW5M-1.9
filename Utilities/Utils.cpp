...
const char *va_print(const char *fmt, ...)
{
	va_list ap;
	char *dest = &g_vaBuffer[g_vaNextBufferIndex][0];
	g_vaNextBufferIndex = (g_vaNextBufferIndex + 1) % VA_BUFFER_COUNT;
...
void Info_SetValueForKey( char *s, const char *key, const char *value ) {
	...
	for(; *blacklist; ++blacklist)
	{
		...
		{
			Log::Error("Info_SetValueForKey", "Can't use keys or values with a '%c': %s = %s\n", *blacklist, key, value);
...
DWORD ReverseIPAddress(DWORD Input)
{
	DWORD result;

	BYTE A = (Input & 0x000000ff);
	BYTE B = (Input & 0x0000ff00) >> 8;
	BYTE C = (Input & 0x00ff0000) >> 16;
	BYTE D = (Input & 0xff000000) >> 24;

	result = (A & 0xFF) * 0x1000000 ^ (B & 0xFF) * 0x10000 ^ (C & 0xFF) * 0x100 ^ (D & 0xFF);
