...
void DumpHandler::Initialize()
{
	...
	WriteIATAddress("winmm.dll", "timeGetTime", (uint64_t)GetModuleHandle(NULL), DumpHandler::SafeTimeGetTime);
}
...
LONG WINAPI DumpHandler::CustomUnhandledExceptionFilter(LPEXCEPTION_POINTERS ExceptionInfo)
{
	...
	strftime(filename, sizeof(filename) - 1, "SteamAPI - %H-%M-%S %d.%m.%Y.dmp", ltime);
	...
	else if (ExceptionInfo->ExceptionRecord->ExceptionCode == 0xC0000005)
	{
		//MessageBox(0, "False exception, dump has been written!", "ERROR", MB_ICONERROR);
		return 0;
	}
	else
	{
		MessageBox(0, va("Fatal error (0x%08x) at 0x%08x.\n%s", ExceptionInfo->ExceptionRecord->ExceptionCode, ExceptionInfo->ExceptionRecord->ExceptionAddress, error), "ERROR", MB_ICONERROR);
	}
...
{
	return std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now() - StartupPoint).count();
}