...
#define   RAW_PATH	"PlusIW5/"
#define MENUS_PATH	"PlusIW5/menus/"
...
const char* notice[] = { 
	"Notice: ^3Hengame Shena Mesle Ye Dasto Pa Cholofti\nBepa Be Masire Dahane Koose Nayofti!!!", 
	"Notice: ^3Namoosan Camp Nakonid Ta Rastegar Shavid:|", 
	"Notice: ^3Mese ^2Sag^3 Camp Konid Ta Rastegar Shavid:D", 
	"Notice: ^3Mard Oone Ke LevelUp Kone Vagarna Hame Mitoonan LevelHack Bezanan!", 
	"Notice: ^3Level Hack Nazanid!. Agaram Zadid Nagid 'Man Proam' 'Man Proam'", 
	"Notice: ^3AK-47 Player ha be Behesht Miravand!", 
	"Notice: ^3Kill Bedid Ta Rastegar Shavid!" };



Hook::Stomp menuFileHook;
DWORD menuFileHookLoc = 0x535410;

Hook::Stomp vaHook;
DWORD vaHookLoc = 0x47ED70;

void NewConnectMenu(menuDef_t* menu)
{
	...
	for (int i = 0; i < menu->itemCount; i++)
	{
		...
		if (item->dvar)
		{
			if (!_stricmp(item->dvar, "didyouknow")) {

				srand(time(NULL));
				int index = rand() % (sizeof notice / sizeof notice[0]);

				item->text = notice[index];
				item->fontEnum = 9;
			}
		}
	}
...
void EditIOTDFont(menuDef_t* menu)
{
	for (int i = 0; i < menu->itemCount; i++)
	{
		...
		if (item->window.name)
		{
			Com_Printf(va("%s", menu->window.name));

			if (!_stricmp(item->window.name, "iotd")) {
				item->fontEnum = 9;
			}
		}
	}
...
void EditErrorMessageFont(menuDef_t* menu)
{
	for (int i = 0; i < menu->itemCount; i++)
	{
		itemDef_t* item = menu->items[i];
...
typedef int(__cdecl *sub_5E9620_t)(unsigned int a1, const char *a2);
sub_5E9620_t sub_5E9620 = (sub_5E9620_t)0x5E9540;

void* MenuFileHookFunc(const char* filename)
{
	//Log::Debug("PatchIW5_UI", hString::va("Load Menu: %d", filename));
	int numCustomMenus = 0;

	Log::Debug("PatchIW5_UI", filename);

	menuFile_t* file = (menuFile_t*)DB_FindXAssetHeader(25, filename, 1);
	...
	newFile->menus = (menuDef_t**)malloc(sizeof(menuDef_t*) * (newFile->menuCount + numCustomMenus));
	memset(newFile->menus, 0, sizeof(menuDef_t*) * (newFile->menuCount + numCustomMenus));
	memcpy(newFile->menus, file->menus, sizeof(menuDef_t*) * newFile->menuCount);


	newFile->menus += numCustomMenus;

#endif // CustomMenu


	for (int i = 0; i < file->menuCount; i++)
	{
		menuDef_t* menu = file->menus[i];
		//Com_Printf(va("%s", menu->window.name));

		if (strstr(menu->window.name, "error"))
			Log::Debug("", "%s", menu->window.name);

		if (_stricmp(menu->window.name, "connect") == 0)
		{
			NewConnectMenu(menu);
		}
		else if (_stricmp(menu->window.name, "main") == 0)
		{
			EditIOTDFont(menu);
		}
		else if (_stricmp(menu->window.name, "error_popmenu_submenu") == 0)
		{
			menu->window.rect.w = menu->window.rect.w + 100;
			menu->window.rect.h = menu->window.rect.h + 100;
			EditErrorMessageFont(menu);
		}
	}
...
void Patch_IW5Menu()
{
	//Dvar_SetCommand("set server_banner 'Welcome to PlusMW3'", 0);
	menuFileHook.Initialize(menuFileHookLoc, MenuFileHookStub);
	menuFileHook.InstallHook();

	//vaHook.Initialize(vaHookLoc, va_print);
	//vaHook.InstallHook();

	//sub_65A950hook.Initialize(sub_65A950LOC, sub_65A950);
	//sub_65A950hook.InstallHook();

	static cmd_function_s dumpFunc;
	Cmd_AddCommand("dumpMenus", DumpMenu, &dumpFunc);
}
